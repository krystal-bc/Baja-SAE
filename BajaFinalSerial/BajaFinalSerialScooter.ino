...
bool recording;
bool ignoreDelay = false;   // used for button press delay (in btnIsPushed() fxn)
int ignoreCount = 0;        // used for button press delay (in btnIsPushed() fxn)
int btnCounter1;
...
void loop() {
  now = rtc.now();
  if (buttonIsPushed(buttonRecord)) {
    manageFile();
  }
  readForces();
  readHallEffects();
...
bool buttonIsPushed(int btn) {
  bool wasPushed = false;
  if ((analogRead(btn) < 500) && (!ignoreDelay)){            // if button is pressed
    wasPushed = true;                                                 // set flag to "was pushed"
    ignoreDelay = true;                                               // start ignoring button presses for 1 sec
    ignoreCount = 0;                                                  // reset ignore timer
    return wasPushed;                                                 // return a single "was pushed" value 
  }
  if ((ignoreDelay) && (ignoreCount <= 1000)){                        // if ignoring and ignore time <= 1000ms
    ignoreCount++;                                                    // increment ignore timer
    wasPushed = false;                                                // reset flag to "was not pushed"
  }
  if ((ignoreDelay) && (ignoreCount > 1000)){                         // if ignoring and ignore time > 1000ms
    ignoreDelay = false;                                              // stop ignoring button presses
  }
  return wasPushed;                                                   // return ignore state at any given instance
}
